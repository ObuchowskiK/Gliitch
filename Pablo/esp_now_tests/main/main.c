...
void vGameLogicTask(void *pvParameters) {
    ...
    bool ath_btn_last_state = false;  // zbocze czarnego w menu athletics
    bool ath_white_last_state = false; // zbocze białego w menu athletics
    ...
    while (1) {
        ...
        while (xQueueReceive(xGamepadQueue, &input_payload, 0) == pdTRUE) {
            ...
            if (p_id >= 1 && p_id <= MAX_PLAYERS) {
                ...
                // --- ATHLETICS MENU INPUT ---
                else if (current_state == STATE_ATHLETICS_MENU && p_id == leader_player_id) {
                    bool black_press = (input_payload.buttons & (1 << 3)) && !ath_btn_last_state;
                    bool white_press = (input_payload.buttons & (1 << 0)) && !ath_white_last_state;
                    ath_btn_last_state   = (input_payload.buttons & (1 << 3));
                    ath_white_last_state = (input_payload.buttons & (1 << 0));
                    if (black_press) { menu_selection = 0; current_state = STATE_MAIN_MENU; }
                    else {
                        if (xTaskGetTickCount() - last_menu_move > pdMS_TO_TICKS(200)) {
                            if (input_payload.joy_y > 3000) { menu_selection = (menu_selection + 1) % 3; last_menu_move = xTaskGetTickCount(); }
                            if (input_payload.joy_y < 1000) { menu_selection = (menu_selection - 1 + 3) % 3; last_menu_move = xTaskGetTickCount(); }
                        }
                        if (white_press) {
                            athletics_init(menu_selection);
                            current_state = STATE_GAME_ATHLETICS;
                        }
                    }
                }
                // --- ATHLETICS GAME INPUT ---
                else if (current_state == STATE_GAME_ATHLETICS) {
                    bool exit = athletics_process_input(p_id, input_payload.joy_x, input_payload.joy_y, input_payload.buttons, p_id == leader_player_id);
                    if (exit) { menu_selection = 0; ath_btn_last_state = true; ath_white_last_state = true; current_state = STATE_ATHLETICS_MENU; }
                }
            ...
            // --- MENU BODY ---
                if (menu_mode == 0) {
                    ...
                    canvas_draw_text(pCurrentDrawCanvas, 140, 290, "WHITE = SELECT", TEXT_GRAY, 1);
                        ...
                        if (games_selection >= visible_items)
                            scroll_offset = games_selection - visible_items + 1;

                        for (int i = 0; i < visible_items; i++) {
                            int game_index = scroll_offset + i;
                            if (game_index >= total_games) break;

                            bool sel = (games_selection == game_index);
        ...
        // ---- ATHLETICS MENU ----
        else if (current_state == STATE_ATHLETICS_MENU) {
            canvas_draw_text(pCurrentDrawCanvas, 160, 30, "ATHLETICS", NEON_PINK, 3);
            const char *ath_games[] = { "100M SPRINT", "LONG JUMP", "HURDLES" };
            for (int i = 0; i < 3; i++) {
                bool sel = (menu_selection == i);
                canvas_draw_text(pCurrentDrawCanvas, 100, 110 + (i * 55),
                    sel ? "> " : "  ", sel ? NEON_CYAN : PURE_WHITE, 3);
                canvas_draw_text(pCurrentDrawCanvas, 148, 110 + (i * 55),
                    ath_games[i], sel ? NEON_CYAN : PURE_WHITE, 3);
            }
            canvas_draw_text(pCurrentDrawCanvas, 60, 290, "WHITE=SELECT  BTN4=BACK", TEXT_GRAY, 1);
        }
